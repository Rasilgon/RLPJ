...
Lets choose a mainDir and have a look at it. Remember that the mainDir is the folder in the directory structure for the parallel runs will be created. It should contain a link to your guess executable and file with list of grid cells.
...
After the steps above, we are ready to call runLPJ. LPJ-GUESS it is not part of this 
...
```
This call will run the model wiht the default values for global_cf template. We can specify other values 
for the modle parameters and also choose what model output we want to create. In the next section, we will do when 
...
Because we are  running this on a personal laptop, we will low number cores and create a SOCK cluster. By calling the
...
If we check the mainDir, we see that there new folders and templates. Within the the runDir folders, there is also a outDir folder to temporary stop the model outputs. The runDirs are still empty. No worries, the LPJSetup onbject contains everything need for each run. Once we call the *runLPJ* things will go smoothly.
```{r}
list.files(mainDir) 
...
Now, we can move to defining the model outputs and the parameters to be tested. 
...
Now we are ready to call the *runLPJ* , but LPJ-GUESS it is not part of this 
package, and thus, calling the function will return an error. Anyway, we should procceed as follows. Note that we use the previous model settings.
...
The runLPJ call would return something like this:
...
Here you will see how a single run takes place. The *runLPJ* does some preliminary taks such as organizing input data into the runDirs. If parallel, it creates the desired cluster. In both casses, runLPJ calls a internal wrapper (*runLPJwrapper*) which agglutinates all single steps to carry one complete call to LPJ-GUESS. Normally, we would call the function, but since we do want to call the LPJ model, we will run the steps that takes in place in a single run.
...
LPJout <- getData(typeList = typeList, outDir=outDir, runInfo=list(runNumber = 1, template1 = "europe.ins", template2="europe_cf.ins", runDir = "runDirectory1"))
...
plotLPJData(data = LPJout, typeList = c("lai") , outDir = outDir, save.plots = FALSE)
```
